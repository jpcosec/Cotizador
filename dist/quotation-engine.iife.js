...
${item.derived.catalogDisaggregated}`,
  ...
  function resolveItemDefinition(itemId, db, visited = new Set()) {
  ...
  function cleanupSubscription$4(subscription) {
  ...
  function destroyRuntimeEntries$1(runtimeEntries) {
    for (const entry of runtimeEntries.values()) {
      cleanupSubscription$4(entry.subscription);
      entry.actor?.stop?.();
    }
  ...
  function stopCategoryRuntime(runtimeByCategoryId, categoryId) {
    ...
    cleanupSubscription$3(runtime.subscription);
    runtime.actor?.stop?.();
  ...
  function destroyRuntimeEntries(runtimeEntries) {
    for (const entry of runtimeEntries.values()) {
      cleanupSubscription$2(entry.subscription);
      entry.actor?.stop?.();
    }
    runtimeEntries.clear();
  }
  ...
  function createBasketActor({
    ...
    function stopAllDayRuntimes() {
      for (const runtime of runtimeByDayIndex.values()) {
        cleanupSubscription$1(runtime.subscription);
        runtime.actor?.stop?.();
      }
  ...
  function toPositiveInteger$1(value, fallback) {
    const parsed = Number(value);
    if (!Number.isFinite(parsed)) return fallback;
    return Math.max(1, Math.floor(parsed));
  }

  function toNullableNumber$1(value) {
  ...
  function normalizeDays(days = []) {
    const indexedDays = (days || []).map((day, index) => ({
      day,
      order: index,
      dayIndex: toPositiveInteger$1(day?.dayIndex, index + 1),
    }));
  ...
  function createDefaultIdPolicy() {
    return {
  ...
  function serializeQuotation({
    ...
    const cotizacion = {
      ...
      Duracion_Dias: toPositiveInteger$1(settings.duracionDias, 1),
      Pax_Global: toPositiveInteger$1(settings.paxGlobal, 1),
      Updated_At: nowIso,
    };
    ...
    const lineas = entries.map(({ dayIndex, entry }, lineIndex) => {
      ...
      return {
        ...
        Override_Pax: toNullableNumber$1(overrides.pax),
        Override_Cantidad: toNullableNumber$1(overrides.cantidad),
        Override_Duracion_Min: toNullableNumber$1(overrides.duracionMin),
  ...
  function settingsFromPayload(cotizacion, lineas, fallbackSettings = {}) {
    ...
    const firstHour = sortedLineas.find((linea) => toId$1(linea?.Hora_Inicio))?.Hora_Inicio;
  ...
  function overridePatchFromLinea(linea) {
    ...
    const hora = toId$1(linea?.Hora_Inicio);
  ...
  function createPersistedQuotationRuntime({
    createRuntime,
    persistencePort,
    idPolicy = null,
  } = {}) {
    ...
    function applyLoadedLine(runtimeInstance, linea, duration) {
      const itemId = toId$1(linea?.ID_Item);
      if (!itemId) {
    ...
    function hydrateFromLoadData(data) {
      ...
      const targetClientId = toId$1(cotizacion.ID_Cliente);
      const selected = (nextSnapshot.clients || []).find((client) => toId$1(client.id) === targetClientId);
    ...
    const api = {
      ...
      async confirmSave() {
        ...
        try {
          ...
          const quotationId =
            toId$1(result.data?.quotationId) ||
            toId$1(result.data?.id) ||
            toId$1(payload.cotizacion.ID_Cotizacion);
      ...
      async loadQuotation(id) {
        ...
        try {
          ...
          flowActor.send({
            type: 'LOAD_DONE',
            quotationId: toId$1(result.data?.quotationId) || quotationId,
  ...
  // Source of truth for the data structure of the system.
  // Synced from claps_codelab/src/Config/Config_Schema.js
  // Version: 3.2.0
  ...
  const DATA_SCHEMA = {
    ...
    COTIZACIONES: {
      description: 'Encabezados de venta.',
      columns: [
    ...
    LINEA_DETALLE: {
      ...
      columns: [
        ...
        { name: 'Estado_Linea', type: 'ENUM', options: ['ACTIVA', 'REMOVIDA'], desc: 'Estado de ciclo de vida' },
        { name: 'Dia_Numero', type: 'INTEGER', desc: 'Día relativo (1..N)' },
        { name: 'Hora_Inicio', type: 'TIME', desc: 'Inicio servicio' },
        { name: 'Override_Pax', type: 'INTEGER', desc: 'Pax override por usuario' },
        { name: 'Override_Cantidad', type: 'DECIMAL', desc: 'Cantidad override por usuario' },
        { name: 'Override_Duracion_Min', type: 'INTEGER', desc: 'Duración override en minutos' },
        { name: 'Comentarios', type: 'TEXT', desc: 'Observaciones' },
        { name: 'Updated_At', type: 'DATETIME', desc: 'Última modificación' }
      ]
    },

    AJUSTES_COTIZACION: {
      description: 'Intervenciones manuales del usuario sobre los cálculos automáticos.',
      columns: [
        { name: 'ID_Ajuste', type: 'PK', desc: 'Identificador' },
        { name: 'ID_Cotizacion', type: 'FK', ref: 'COTIZACIONES', desc: 'Cotización afectada' },
        { name: 'ID_Linea', type: 'FK', ref: 'LINEA_DETALLE', desc: 'Línea afectada' },
    ...
    CACHE_COTIZACION: {
      description: 'JSON snapshot de resultados calculados.',
      columns: [
        { name: 'ID_Cotizacion', type: 'PK/FK', ref: 'COTIZACIONES', desc: '1:1 con cotización' },
        { name: 'Snapshot_JSON', type: 'JSON', desc: 'Resultado completo' },
  ...
  function inferPrimaryKey$1(columns) {
  ...
  function buildModel({ tableName, schema, store }) {
    ...
    const primaryKey = inferPrimaryKey$1(columns);
   ...
   */
  function createDatabase({ adapter = 'memory', adapterOptions = {}, schema = DATA_SCHEMA, seed = [] } = {}) {
    if (adapter !== 'memory') {
  ...
  function persistenceOk(data) {
    return { ok: true, data };
  }

  function persistenceError(code, message, details = null) {
  ...
  class GasSheetAdapter extends PersistencePort {
    constructor({
      invoke = null,
  ...
  class LocalPersistenceAdapter extends PersistencePort {
    constructor({ models } = {}) {
      super();
  ...
  // AUTO-GENERATED by gas/scripts/generate_local_init_tables.mjs
  ...
  const LOCAL_INIT_TABLES = {
    ...
    "COMPOSICION_KIT": [
      ...
      {
        "ID_Composicion": "KIT_COFFEE_BASICO_MEDIALUNA",
        "ID_Item_Padre": "ITEM_COFFEE_BASICO",
        "ID_Item_Hijo": "ITEM_1_MEDIALUNA",
        "Cantidad": 1,
        "Tipo_Precio": "ABSORBIDO",
        "Updated_At": "2026-04-20T00:00:00.000Z"
      }
  ...
  function exportQuotationToCsv(validationRows, totals) {
  ...
  function clonePlain$2(value) {
    if (value === null || typeof value !== 'object') {
      return value;
    }

    if (Array.isArray(value)) {
      return value.map((entry) => clonePlain$2(entry));
    }
    ...
    for (const [key, entry] of Object.entries(value)) {
      ...
      result[key] = clonePlain$2(entry);
    }

    return result;
  }

  function mergePlain$1(base = {}, patch = {}) {
    return {
      ...clonePlain$2(base),
      ...clonePlain$2(patch),
    };
  ...
  class GenericUnitBase extends RuntimeUnitMixin(class {}) {
    constructor(options = {}) {
      super();
      ...
      this.errors = [];
      this.warnings = [];
      this.meta = clonePlain$2(options.meta ?? {});
      this.boundaryStatus = {};
    ...
    initialize(context = {}, boundaries = {}, initialState = {}) {
      this.context = clonePlain$2(context);
      this.boundaries = { ...boundaries };
      this.state = mergePlain$1(this.state, initialState);
      this.status = 'ready';
      this.refresh();
    ...
    registerChild(unit, meta = {}) {
      ...
      const childMeta = clonePlain$2(meta);
      unit.setParent(this);
    ...
    applyMutation(mutation = {}) {
      const normalized = clonePlain$2(mutation);
      ...
      if (Object.keys(contextPatch).length > 0) {
        this.context = mergePlain$1(this.context, contextPatch);
      }

      if (Object.keys(patch).length > 0) {
        this.state = mergePlain$1(this.state, patch);
      }

      if (normalized.ui) {
        this.ui = mergePlain$1(this.ui, normalized.ui);
      }

      if (normalized.derived) {
        this.derived = mergePlain$1(this.derived, normalized.derived);
      }

      if (normalized.boundaryStatus) {
        this.boundaryStatus = mergePlain$1(this.boundaryStatus, normalized.boundaryStatus);
      }

      if (Array.isArray(normalized.errors)) {
        this.errors = clonePlain$2(normalized.errors);
      }

      if (Array.isArray(normalized.warnings)) {
        this.warnings = clonePlain$2(normalized.warnings);
      }
    ...
    getProjection() {
      return {
        ...
        stage: this.stage,
        state: clonePlain$2(this.state),
        context: clonePlain$2(this.context),
        ui: clonePlain$2(this.ui),
        children: this.getChildEntries().map(({ unit, meta }) => ({
          ...unit.getProjection(),
          meta,
        })),
        derived: clonePlain$2(this.derived),
        errors: clonePlain$2(this.errors),
        warnings: clonePlain$2(this.warnings),
        actions: clonePlain$2(this.ui.actions ?? []),
        boundaryStatus: clonePlain$2(this.boundaryStatus),
        lastSignal: clonePlain$2(this.lastSignal),
      };
    }
  ...
  function clonePlain$1(value) {
    if (value === null || typeof value !== 'object') {
      return value;
    }

    if (Array.isArray(value)) {
      return value.map((entry) => clonePlain$1(entry));
    }
  ...
  function createViewMachine(view) {
    ...
    return setup({
      actions: {
        requestSave: ({ event, self }) => {
          void view.callBoundary('persistence', 'save', {
            ...
            payload: clonePlain$1(event.payload ?? {}),
          }, { actorRef: self });
        },
      ...
      context: {
        ...
        contextData: clonePlain$1(view.context),
        runtimeState: clonePlain$1(view.state),
        ui: clonePlain$1(view.ui),
        boundaryStatus: clonePlain$1(view.boundaryStatus),
        derived: clonePlain$1(view.derived),
        errors: clonePlain$1(view.errors),
        warnings: clonePlain$1(view.warnings),
        status: view.status,
        lastSignal: null,
      },
      states: {
      ...
      on: {
        [RuntimeSignal.applyMutation]: {
          actions: assign(({ context, event }) => ({
            runtimeState: mergePlain(context.runtimeState, clonePlain$1(event.patch ?? event.state ?? event.mutation?.patch ?? {})),
            contextData: mergePlain(context.contextData, clonePlain$1(event.contextPatch ?? event.mutation?.contextPatch ?? {})),
            ui: mergePlain(context.ui, clonePlain$1(event.ui ?? event.mutation?.ui ?? {})),
            derived: mergePlain(context.derived, clonePlain$1(event.derived ?? event.mutation?.derived ?? {})),
            status: event.status ?? event.mutation?.status ?? context.status,
            lastSignal: clonePlain$1(event),
          })),
        },
        [RuntimeSignal.patchContext]: {
          actions: assign(({ context, event }) => ({
            contextData: mergePlain(context.contextData, clonePlain$1(event.patch ?? event.contextPatch ?? {})),
            lastSignal: clonePlain$1(event),
          })),
        },
        [RuntimeSignal.setContext]: {
          actions: assign(({ event }) => ({
            contextData: clonePlain$1(event.patch ?? event.contextPatch ?? {}),
            lastSignal: clonePlain$1(event),
          })),
        },
          ...
          actions: assign(({ context, event }) => ({
            boundaryStatus: mergePlain(context.boundaryStatus, {
              [`${event.boundaryName}.${event.actionName}`]: {
                status: 'success',
                result: clonePlain$1(event.result),
              },
            }),
            derived: mergePlain(context.derived, {
              [event.boundaryName]: clonePlain$1(event.result),
            }),
            status: 'ready',
            lastSignal: clonePlain$1(event),
          })),
        ...
        [RuntimeSignal.childSignal]: {
          actions: assign(({ context, event }) => ({
            runtimeState: mergePlain(context.runtimeState, {
              childSignals: [...(context.runtimeState.childSignals ?? []), clonePlain$1(event.signal)].slice(-10),
            }),
            lastSignal: clonePlain$1(event),
          })),
        },
  ...
  class GenericViewBase extends GenericUnitBase {
    constructor(options = {}) {
      super({
        ...options,
    ...
    onActorUpdate(snapshot) {
      ...
      this.context = clonePlain$1(machineContext.contextData ?? this.context);
      this.state = clonePlain$1(machineContext.runtimeState ?? this.state);
      this.ui = clonePlain$1(machineContext.ui ?? this.ui);
      this.boundaryStatus = clonePlain$1(machineContext.boundaryStatus ?? this.boundaryStatus);
      this.derived = clonePlain$1(machineContext.derived ?? this.derived);
      this.errors = clonePlain$1(machineContext.errors ?? this.errors);
      this.warnings = clonePlain$1(machineContext.warnings ?? this.warnings);
      this.status = machineContext.status ?? this.status;
      this.lastSignal = clonePlain$1(machineContext.lastSignal ?? this.lastSignal);
      this.refresh();
    }
    ...
    getProjection() {
      ...
      const visibleUnits = visibleChildren.map(({ unit, meta }) => ({
        ...
        stageIds: clonePlain$1(meta.stages ?? []),
      }));
  ...
  class QuotationFlowRuntimeView extends GenericViewBase {
  ...
  function createQuotationFlowRuntimeView(runtime, options = {}) {
